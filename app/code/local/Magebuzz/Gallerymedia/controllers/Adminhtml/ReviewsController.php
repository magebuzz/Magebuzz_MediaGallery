...
class Magebuzz_Gallerymedia_Adminhtml_ReviewsController extends Mage_Adminhtml_Controller_action
{
  ...
  protected function _initAction()
  {
    $this->loadLayout()
      ...
      ->_addBreadcrumb(Mage::helper('adminhtml')->__('Reviews Manager'), Mage::helper('adminhtml')->__('Reviews Manager'));
  ...
  public function editAction()
  {
    ...
    $model = Mage::getModel('gallerymedia/reviews')->load($id);
    ...
    if ($model->getId() || $id == 0) {
      ...
      $this->_addContent($this->getLayout()->createBlock('gallerymedia/adminhtml_reviews_edit'))
        ->_addLeft($this->getLayout()->createBlock('gallerymedia/adminhtml_reviews_edit_tabs'));
  ...
  {
    $this->loadLayout();
    $this->getResponse()->setBody($this->getLayout()->createBlock('gallerymedia/adminhtml_reviews_grid')->toHtml());
  }
  ...
  public function saveAction()
  {
    if ($data = $this->getRequest()->getPost()) {
      ...
      $model = Mage::getModel('gallerymedia/reviews');
      $model->setData($data)
  ...
  {
    $gallerymediaIds = $this->getRequest()->getParam('review');
    if (!is_array($gallerymediaIds)) {
      Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
    } else {
      try {
        foreach ($gallerymediaIds as $gallerymediaId) {
          $gallerymedia = Mage::getModel('gallerymedia/reviews')->load($gallerymediaId);
          $gallerymedia->delete();
        }
  ...
  public function massStatusAction()
  {
    ...
    if (!is_array($gallerymediaIds)) {
      ...
      try {
        foreach ($gallerymediaIds as $gallerymediaId) {
          $gallerymedia = Mage::getSingleton('gallerymedia/reviews')
            ->load($gallerymediaId)
  ...
  protected function _sendUploadResponse($fileName, $content, $contentType = 'application/octet-stream')
  {
	...
	protected function _isAllowed() {
		return Mage::getSingleton('admin/session')->isAllowed('gallerymedia/reviews');
	}
}
